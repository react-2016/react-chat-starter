...
Firechat.prototype.setUser = function(userId, userName, callback) {
    ...
    self._firebase.onAuth(function(authData) {
        if (authData) {
            ...
            self.warn('Firechat requires an authenticated Firebase reference. ' +
...
Firechat.prototype.enterRoom = function(roomId) {
    ...
    self.getRoom(roomId, function(room) {
        ...
        if (self._user) {
            ...
            var presenceRef = self._firebase
                .child('room-users').child(roomId).child(self._userId).child(self._sessionId);
        ...
        self._roomRef.child(roomId).once('value', function(snapshot) {
            self._messageRef.child(roomId)
                    .limitToLast(self._options.numMaxMessages).on('child_added', function(snapshot) {
            ...
            self._messageRef.child(roomId)
                    .limitToLast(self._options.numMaxMessages).on('child_removed', function(snapshot) {
...
Firechat.prototype.leaveRoom = function(roomId) {
    var self = this;
    var userRoomRef = self._firebase.child('room-users').child(roomId);
...
Firechat.prototype.sendSuperuserNotification = function(userId, notificationType, data, cb) {
    var self = this;
    var userNotificationsRef = self._firebase.child('users').child(userId).child('notifications');
...
Firechat.prototype.getUsersByRoom = function() {
    ...
    var query = self._firebase.child('room-users').child(roomId);
